CREATE OR REPLACE package body mai2325 as
-----------------------------------------------------------------------------
--
--   PVCS Identifiers :-
--
--       sccsid           : $Header:   //vm_latest/archives/mai/admin/pck/mai2325.pkb-arc   2.1   Jul 01 2013 16:26:48   James.Wadsworth  $
...
is
       ...
       is select 'insert into mai2325_results
                  ( mai2325_job_no
                   ...
                   ,mai2325_inventory
                   ,mai2325_attribute
                   ,mai2325_attribute_value
                   ,mai2325_number
                   ,mai2325_length
                   ,mai2325_area
                  ) select '||to_char(v_job_no)||'
               '||',rse.rse_he_id
...
' group by rse.rse_he_id,initcap(ity.ity_descr),initcap(ita.ita_scrn_text)'   v_query
...
select 'insert into mai2325_results
               ...
               '||',rse.rse_he_id
                 ...
                 ,'||''''||p2.rep_p5||''''||'
                 ,count(iit.iit_rse_he_id)
                 ,sum(nvl(iit.iit_st_chain,0)-nvl(iit.iit_st_chain,0))
                 ,sum(nvl(iit.iit_width,0))
        from '||ity2.ity_view_name||' iit
             ,inv_item_types       ity
             ,inv_type_attribs     ita
             ,road_sections_all    rse
       ...
       is select rse_he_id
          ...
          where  exists
                  ( select 'exists'
                    from   report_params p1
                    where  p1.rep_sessionid     = v_job_no
                    and    p1.rep_p2 = to_char(rse_he_id));
    --
begin
    --
    ...
    then for each_group_section in all_group_sections
         loop
           ...
           loop
              ...
              if dbug
              then insert into mai2325_query (mai2325_job_no,mai2325_query) values (v_job_no,v_query);
