CREATE OR REPLACE PACKAGE BODY interfaces IS
-----------------------------------------------------------------------------
--
--   PVCS Identifiers :-
--
--       sccsid           : $Header:   //vm_latest/archives/mai/admin/pck/interfaces.pkb-arc   2.3.1.3   Nov 20 2008 10:16:10   smarshall  $
--       Module Name      : $Workfile:   interfaces_4043.pkb  $
--       Date into SCCS   : $Date:   Nov 20 2008 10:16:10  $
--       Date fetched Out : $Modtime:   Oct 15 2008 15:46:58  $
--       SCCS Version     : $Revision:   2.3.1.3  $
...
--
  ...
  g_body_sccsid  CONSTANT varchar2(2000) := '$Revision:   2.3.1.3  $';
...
FOR c1rec IN boq1_n LOOP --all the parent boq items for this wol
  ...
  FOR c2rec IN percentage1_n(c1rec.icboq_boq_id) LOOP --all the children of each parent
    ...
    IF c2rec.icboq_percent_band_comp = 'N' THEN
      UPDATE interface_claims_boq
      SET    icboq_error = SUBSTR(icboq_error||'1) Bill Item cost does not equal the product of its quantity and rate. '||c2rec.icboq_boq_id||'/'||c2rec.icboq_percent_band_comp||'('||abs(icboq_cost - ROUND((icboq_rate/100)*(c1rec.icboq_percent_adjust),2))||')', 1, 254)
            ,icboq_status = 'R'
	...
	ELSE
	  IF l_count = 1 THEN
 	  --the 1st % item is calculated against the item cost regardless.
        UPDATE interface_claims_boq
        SET    icboq_error = SUBSTR(icboq_error||'2) Bill Item cost does not equal the product of its quantity and rate. '||c2rec.icboq_boq_id||'/'||c2rec.icboq_percent_band_comp||'('||abs(icboq_cost - ROUND((icboq_rate/100)*(c1rec.icboq_percent_adjust),2))||')', 1, 254)
              ,icboq_status = 'R'
        WHERE  0.03 <= abs(icboq_cost - ROUND((icboq_rate/100)*(c1rec.icboq_percent_adjust),2))--SM 13082008 715407 introduced a 0.03 tolerance
        AND    icboq_ih_id = p_ih_id
	AND    icboq_boq_id = c2rec.icboq_boq_id
	  ...
	  --% items other than the 1st are calculated against the item cost plus the previous % costs.
        UPDATE interface_claims_boq
        SET    icboq_error = SUBSTR(icboq_error||'3) Bill Item cost does not equal the product of its quantity and rate. '||c2rec.icboq_boq_id||'/'||c2rec.icboq_percent_band_comp||'('||abs(icboq_cost - ROUND((icboq_rate/100)*(c1rec.icboq_percent_adjust),2))||')', 1, 254)
              ,icboq_status = 'R'
        WHERE  0.03 <= abs(icboq_cost - ROUND((icboq_rate/100)*(c1rec.icboq_percent_adjust),2))--SM 13082008 715407 introduced a 0.03 tolerance
        AND    icboq_ih_id = p_ih_id
	AND    icboq_boq_id = c2rec.icboq_boq_id
...
IF p_ih_id IS NOT NULL
  THEN
    ...
    IF l_interface_header.ih_file_type = 'WC'
      THEN
        interfaces.validate_completion_data(p_ih_id);--validate_completion;
    ELSE
    ...
    CLOSE c2;
      ...
      IF l_interface_header.ih_status = 'P'
          ...
          IF l_interface_header.ih_file_type = 'WC'
            THEN
              interfaces.completion_file_ph2(p_ih_id);
...
PROCEDURE auto_load_file(p_ih_id	IN OUT interface_headers.ih_id%TYPE
  ...
  CURSOR c2 (p_ih_id interface_claims_wor_all.icwor_ih_id%TYPE) IS
    SELECT icwor_works_order_no, icwor_claim_value
    FROM interface_claims_wor_all
    WHERE p_ih_id = icwor_ih_id
    AND icwor_error IS NULL;

  CURSOR c3 (p_ih_id interface_claims_wor_all.icwor_ih_id%TYPE) IS
    SELECT COUNT(1)
    FROM interface_claims_wor_all
    WHERE p_ih_id = icwor_ih_id
    AND icwor_error IS NULL;
...
IF p_ih_id IS NOT NULL
  THEN
    ...
    IF l_count > 0 THEN
      FOR c2rec IN c2(p_ih_id) LOOP --open interface_claims_wor_all using the ih_id (could return a few rows)
        submit_record ( p_ih_id, c2rec.icwor_works_order_no, c2rec.icwor_claim_value );
      END LOOP;
    ELSE
    ...
    LOOP
	  ...
	  IF l_no_of_recs != int_utility.get_field(l_record, 2) OR
	     l_total_value != int_utility.get_field(l_record, 3) THEN
          ...
          --
          IF interfaces.g_job_id IS NOT NULL THEN
            higgrirp.write_gri_spool(interfaces.g_job_id,'WARNING: Check record totals do not tally.');
          END IF;
          --
        END IF;
