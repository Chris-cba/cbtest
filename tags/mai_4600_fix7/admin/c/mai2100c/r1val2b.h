...
/* "$Workfile:   r1val2b.h  $ $Revision:   2.2  $ $Modtime:   Dec 04 2012 20:18:42  $" */
...
BOOLEAN slinkvl(link,lineno)
  ...
  if (strlen(link) > 6)
    {
    strcpy(err_msg,"ERROR: Link code format - More than 6 characters BPR-8016");
    format_err(lineno,1);              
    }   

  if (strlen(link)  < 6)
    {
    strcpy(err_msg,"ERROR: Link code format - Less than 6 characters BPR-8017");
    format_err(lineno,1);              
    }   
    ...
    if (isalnum(link[i]))
      ...
      if (link[i] == ' ' && i != 0)
        ...
        if (link[i] == ' ')
          ...
          {
          strcpy(err_msg,"ERROR: Link code format - Invalid character BPR-8019");
          format_err(lineno,1);              
          }      
        }
      }
...
external action : output to stdout appropriate error message.
                  and line number . via global string err_msg. 
*/
BOOLEAN ssectvl(section,lineno)
  /* section field text */
  char *section;           
  /* line number */
  long int lineno;
  ...
  if (strlen(section)  < 5)
    {
    strcpy(err_msg,"ERROR: Section code format - Less than 5 characters BPR-8021");
    format_err(lineno,1);              
    }   
...
BOOLEAN sdatevl(date,lineno)
  ...
  if (strlen(date) > 8)
    {
    strcpy(err_msg,"ERROR: Date format - More than 8 characters BPR-8023");
    format_err(lineno,1);              
    }   

  if (strlen(date)  < 8)
    {
    strcpy(err_msg,"ERROR: Date format - Less than 8 characters BPR-8024");
    format_err(lineno,1);              
    }   
  ...
  switch (month)
    ...
    case 2 :    
              ...
              break;
    default :
              strcpy(err_msg,"ERROR: Date format - Invalid Month        BPR-8031");
              format_err(lineno,1);              
              break;
    }
...
BOOLEAN sinspvl(insp,lineno)
  ...
  if (strlen(insp) > 5)
    {
    strcpy(err_msg,"ERROR: Inspector initials  - More than 5 characters BPR-8032");
    format_err(lineno,1);              
    }   

  if (strlen(insp)  < 5)
    {
    strcpy(err_msg,"ERROR: Inspector initials  - Less than 5 characters BPR-8033");
    format_err(lineno,1);              
    }   
