...
/* "$Workfile:   r1validb.h  $ $Revision:   2.2  $ $Modtime:   Dec 04 2012 20:18:42  $" */
...
BOOLEAN b_validation(rtext,lineno)
  ...
  if (((noargs = getwstrs(text,strs)) < 5)||((noargs = getwstrs(text,strs)) > 7))
    {                           
...
BOOLEAN linkvl(link,lineno)
  ...
  if (strlen(link) > 10)
    { 
    strcpy(err_msg,"ERROR: Link code format - More than 10 characters BPR-8016");
    format_err(lineno,1);
    }   

  if (strlen(link)  < 10)
    { 
    strcpy(err_msg,"ERROR: Link code format - Less than 10 characters BPR-8017");
    format_err(lineno,1);
    }   
...
BOOLEAN sectvl(section,lineno)
  ...
  if (strlen(section) > 5)
    {
    strcpy(err_msg,"ERROR: Section code format - More than 2 characters BPR-8020");
    format_err(lineno,1);
    }   
  ...
  if (strlen(section)  < 2)
    {
    strcpy(err_msg,"ERROR: Section code format - Less than 2 characters BPR-8021");
    format_err(lineno,1);
    }   
...
BOOLEAN datevl(date,lineno)
  ...
  if (strlen(date) > 6)
    {
    strcpy(err_msg,"ERROR: Date format - More than 6 characters BPR-8023");
    format_err(lineno,1);
    }   

  if (strlen(date)  < 6)
    {
    strcpy(err_msg,"ERROR: Date format - Less than 6 characters BPR-8024");
    format_err(lineno,1);
    }   
  ...
  for (i=0;i < strlen(date);++i)
    ...
    if (isdigit(date[i]))
      ...
      {
      strcpy(err_msg,"ERROR: Date format - Invalid character BPR-8025");
      format_err(lineno,1);
      }      
    }
  ...
  if (day == 0)
    {
    strcpy(err_msg,"ERROR: Date format - Invalid day BPR-8027");
    format_err(lineno,1);
    }      
  ...
  switch (month)
    ...
    case 2 :    
              ...
              break;
    default : strcpy(err_msg,"ERROR: Date format - Invalid Month        BPR-8031");
              format_err(lineno,1);
              break;
    } /* end brace SWITCH */
...
BOOLEAN inspvl(insp,lineno)
  ...
  if (strlen(insp) > 3)
    {
    strcpy(err_msg,"ERROR: Inspector initials  - More than 3 characters BPR-8032");
    format_err(lineno,1);
    }   

  if (strlen(insp)  < 3)
    {
    strcpy(err_msg,"ERROR: Inspector initials  - Less than 3 characters BPR-8033");
    format_err(lineno,1);
    }   
  ...
  for (i=0;i < strlen(insp);++i)
    ...
    if (isalpha(insp[i])||(insp[i]==' '))
      ...
      {
      strcpy(err_msg,"ERROR: Inspectors initials - Invalid character BPR-8034");
      format_err(lineno,1);
      }      
    }
