...
-----------------------------------------------------------------------------
--
--PROCEDURE create_asset(pi_he_id     hhinv_load_3.he_id%TYPE
--                      ,pi_st_chain  hhinv_load_3.st_chain%TYPE
--                      ,pi_end_chain hhinv_load_3.end_chain%TYPE) IS
--  --
--  lv_inv_rowid      UROWID;
--  lv_warning_code   VARCHAR2(80);
--  lv_warning_msg    VARCHAR2(80);
--  lv_temp_ne_id     NUMBER;
--  --
--
...
-- --
--  --nm_debug.debug_on;
--  --nm_debug.debug('Create_assets');
--  IF gr_rec_iit.iit_start_date IS NULL
--   THEN
--      gr_rec_iit.iit_start_date := nm3user.get_effective_date;
--  END IF;
--  --
--  IF gr_rec_iit.iit_primary_key IS NULL
--   THEN
--      gr_rec_iit.iit_primary_key := gr_rec_iit.iit_ne_id;
--  END IF;
--  /*
--  || Derive admin unit from road
--  */
--  IF gr_rec_iit.iit_admin_unit IS NULL
--   THEN
--      gr_rec_iit.iit_admin_unit := g_admin_unit;
--  END IF;
--  --
--
--  ----------------------------------------------------------------------
...
--  -- then ensure that the insert would not violate IIT_UK
--  ----------------------------------------------------------------------
--  IF nm3get.get_iit_all(pi_iit_primary_key   => gr_rec_iit.iit_primary_key
--                       ,pi_iit_inv_type      => gr_rec_iit.iit_inv_type
--                       ,pi_iit_start_date    => gr_rec_iit.iit_start_date
--                       ,pi_raise_not_found   => FALSE).iit_ne_id IS NOT NULL AND gr_rec_iit.iit_end_date IS NULL THEN
--
--    --
--    -- NET-0445: Asset item with this primary key/asset type/start date already exists
--    --
--    hig.raise_ner(pi_appl               => 'NET'
--                 ,pi_id                 => 445
--                 ,pi_supplementary_info => chr(10)||gr_rec_iit.iit_primary_key||chr(10)||gr_rec_iit.iit_inv_type||chr(10)||to_char(gr_rec_iit.iit_start_date,'DD-MON-YYYY'));
--  END IF;
--
--
--------------------------------------------------------------------
---- end date the item if it already exists
---- (it could already exist but with a different
---- start date - which is why it could get thru the above checking)
--------------------------------------------------------------------
--  OPEN  cs_check (gr_rec_iit.iit_primary_key, gr_rec_iit.iit_inv_type);
--  FETCH cs_check
--   INTO lv_inv_rowid;
--  IF cs_check%FOUND
--   THEN
--      UPDATE nm_inv_items
--         SET iit_end_date = nvl(gr_rec_iit.iit_start_date,gr_rec_iit.iit_end_date)
--       WHERE ROWID = lv_inv_rowid
--         AND gr_rec_iit.iit_end_date IS NULL;
--  END IF;
--  CLOSE cs_check;
--  --
--
-------------------------------------
---- Insert new item if not end dated
-------------------------------------
--  IF gr_rec_iit.iit_end_date IS NULL
--   THEN
--      nm3ins.ins_iit_all(gr_rec_iit);
--  END IF;
--  --
--
--  IF gr_rec_iit.iit_end_date IS NOT NULL
--   THEN
----      gr_rec_iit.iit_ne_id :=
----        nm3xml_load.get_existing_inv_item(pi_inv_type   => gr_rec_iit.iit_inv_type
----                                         ,pi_nte_job_id => lv_temp_ne_id
----                                         ,pi_end_date   => gr_rec_iit.iit_end_date);
--
----      lv_temp_ne_id := NM3XML_LOAD.GET_TEMP_EXTENT(pi_iit_ne_id  => gr_rec_iit.iit_ne_id
----                                                 ,pi_nte_job_id => lv_temp_ne_id);
--
--      gr_rec_iit.iit_start_date := gr_rec_iit.iit_END_date;
--
--  END IF;
--  --
--
...
--  ELSE
--      /*
--      || Create A Temp Extent From The Rse_He_Id Of The
--      || Object And Locate The Asset Relative To This.
--      */
--      nm3extent.create_temp_ne(pi_source_id => pi_he_id
--                              ,pi_source    => nm3extent.c_route
--                              ,pi_begin_mp  => pi_st_chain
--                              ,pi_end_mp    => pi_end_chain
--                              ,po_job_id    => lv_temp_ne_id);
--  END IF;
--  --
--  IF lv_temp_ne_id = -1
--   THEN
--      nm3homo.end_inv_location(pi_iit_ne_id            => gr_rec_iit.iit_ne_id
--                              ,pi_effective_date       => gr_rec_iit.iit_start_date
--                              ,po_warning_code         => lv_warning_code
--                              ,po_warning_msg          => lv_warning_msg
--                              ,pi_ignore_item_loc_mand => true);
--  ELSE
--      IF lv_temp_ne_id IS NOT NULL
--       THEN
--          nm3homo.homo_update(p_temp_ne_id_in  => lv_temp_ne_id
--                             ,p_iit_ne_id      => gr_rec_iit.iit_ne_id
--                             ,p_effective_date => gr_rec_iit.iit_start_date);
--      END IF;
--  END IF;
--  --
--END create_asset;
