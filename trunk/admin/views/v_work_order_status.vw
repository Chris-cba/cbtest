...
--
      ...
      ,NVL(wos.wor_status,'DRAFT') wor_status
  FROM work_orders wor
      ,(SELECT wol_works_order_no
              ,CASE WHEN paid = wols       THEN (SELECT hsc_status_code
                    ...
                    WHEN completed = wols  THEN (SELECT hsc_status_code
                                                   FROM hig_status_codes
                                                  WHERE hsc_domain_code = 'WORK_ORDER_LINES'
                                                    AND TRUNC(SYSDATE) BETWEEN NVL(hsc_start_date,TRUNC(SYSDATE))
                                                                           AND NVL(hsc_end_date,TRUNC(SYSDATE))
                    ...
                    WHEN instructed = wols THEN (SELECT hsc_status_code
                                                   FROM hig_status_codes
                                                  WHERE hsc_domain_code = 'WORK_ORDER_LINES'
                                                    AND TRUNC(SYSDATE) BETWEEN NVL(hsc_start_date,TRUNC(SYSDATE))
                                                                           AND NVL(hsc_end_date,TRUNC(SYSDATE))
