...
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //vm_latest/archives/mai/install/mai4010_mai4022_upg.sql-arc   2.1   Jul 01 2013 16:03:02   James.Wadsworth  $
--       Module Name      : $Workfile:   mai4010_mai4022_upg.sql  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai4010_mai4022_1_&log_extension'
define logfile2='mai4010_mai4022_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'MAI'
                          ,p_new_version           => '4.0.2.0'
                          ,p_allowed_old_version_1 => '4.0.1.0'
                          );
END;
/
...
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4010_mai4022_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4010_mai4022_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('MAI','mai4010_mai4022_upg.sql','Upgrade from 4.0.1.0 to 4.0.2.2','4.0.2.2');
END;
/
COMMIT;
SET HEADING OFF
