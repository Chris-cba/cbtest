...
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //vm_latest/archives/mai/install/mai4022_mai4040_upg.sql-arc   2.0   Jan 22 2008 10:25:56   jwadsworth  $
--       Module Name      : $Workfile:   mai4022_mai4040_upg.sql  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai4022_mai4040_1_&log_extension'
define logfile2='mai4022_mai4040_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'MAI'
                          ...
                          ,p_allowed_old_version_1 => '4.0.2.2'
                          );
END;
/
...
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4022_mai4040_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4022_mai4040_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('MAI','mai4022_mai4040_upg.sql','Upgrade from 4.0.2.2 to 4.0.4.0','4.0.4.0');
END;
/
COMMIT;
SET HEADING OFF
