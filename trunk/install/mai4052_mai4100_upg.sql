...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //vm_latest/archives/mai/install/mai4052_mai4100_upg.sql-arc   3.0   Jul 17 2009 18:11:20   mhuitson  $
--       Module Name      : $Workfile:   mai4052_mai4100_upg.sql  $
--       Date into PVCS   : $Date:   Jul 17 2009 18:11:20  $
--       Date fetched Out : $Modtime:   Jul 17 2009 17:43:18  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai4052_mai4100_1_&log_extension'
define logfile2='mai4052_mai4100_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'MAI'
                          ...
                          ,p_allowed_old_version_1 => '4.0.5.2'
                          );
END;
/
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4052_mai4100_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('MAI','mai4052_mai4100_upg.sql','Upgrade from 4.0.5.2 to 4.1.0.0','4.1.0.0');
END;
/
COMMIT;
SET HEADING OFF
