...
--   SCCS Identifiers :-
--
--       sccsid           : $Header:   //vm_latest/archives/mai/install/mai40_mai4010_upg.sql-arc   2.1   Jul 01 2013 16:02:22   James.Wadsworth  $
--       Module Name      : $Workfile:   mai40_mai4010_upg.sql  $
--       Date into SCCS   : $Date:   Jul 01 2013 16:02:22  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai40_mai4010_1_&log_extension'
define logfile2='mai40_mai4010_2_&log_extension'
spool &logfile1
--get some db info
...
SELECT '&exor_base'||'mai'||'&terminator'||'admin'||
        '&terminator'||'views'||'&terminator'||'maiviews' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai40_mai4010_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('MAI','mai40_mai4010_upg.sql','Upgrade from 4.0 to 4.0.1.0','4.0.1.0');
END;
/
COMMIT;
SET HEADING OFF
