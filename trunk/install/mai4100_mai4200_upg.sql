...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //vm_latest/archives/mai/install/mai4100_mai4200_upg.sql-arc   3.4   Jul 01 2013 16:03:26   James.Wadsworth  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai4100_mai4200_1_&log_extension'
define logfile2='mai4100_mai4200_2_&log_extension'
spool &logfile1
--get some db info
...
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4100_mai4200_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4100_mai4200_ddl2_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4100_mai4200_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('MAI','mai4100_mai4200_upg.sql','Upgrade from 4.1.0.0 to 4.2.0.0','4.2.0.0');
END;
/
COMMIT;
SET HEADING OFF
