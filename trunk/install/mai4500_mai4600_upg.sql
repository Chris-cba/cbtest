...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //vm_latest/archives/mai/install/mai4500_mai4600_upg.sql-arc   1.2   Jul 01 2013 16:03:30   James.Wadsworth  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai4500_mai4600_1_&log_extension'
define logfile2='mai4500_mai4600_2_&log_extension'
spool &logfile1
--get some db info
...
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4500_mai4600_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4500_mai4600_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('MAI','mai4500_mai4600_upg.sql','Upgrade from 4.5.0.0 to 4.6.0.0','4.6.0.0');
END;
/
COMMIT;
SET HEADING OFF
