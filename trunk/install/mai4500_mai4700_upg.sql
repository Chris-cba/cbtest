...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //vm_latest/archives/mai/install/mai4500_mai4700_upg.sql-arc   1.0   Dec 16 2013 08:46:18   Chris.Baugh  $
--       Module Name      : $Workfile:   mai4500_mai4700_upg.sql  $
--       Date into PVCS   : $Date:   Dec 16 2013 08:46:18  $
--       Date fetched Out : $Modtime:   Nov 08 2013 13:54:36  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai4500_mai4700_1_&log_extension'
define logfile2='mai4500_mai4700_2_&log_extension'
spool &logfile1
--get some db info
...
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4500_mai4700_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4500_mai4700_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('MAI','mai4500_mai4700_upg.sql','Upgrade from 4.5.0.0 to 4.7.0.0','4.7.0.2');
END;
/
COMMIT;
SET HEADING OFF
