...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='mai4700_mai4800_1_&log_extension'
define logfile2='mai4700_mai4800_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'MAI'
                          ...
                          ,p_allowed_old_version_1 => '4.7.0.3'
                          );
END;
/
...
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4700_mai4800_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'mai'||'&terminator'||'install'||
        '&terminator'||'mai4700_mai4800_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
