...
REM @(#)$Header:   //vm_latest/archives/mai/mig/mai_checks.sql-arc   2.1   Jul 01 2013 16:00:04   James.Wadsworth  $
...
PROMPT
PROMPT CHECKING THAT A CROSS SECTION OF MAI TABLES HAVE BEEN MIGRATED FROM SOURCE 
PROMPT ==========================================================================
...
--
   ...
   FOR i IN 1..l_tab_table_name.COUNT LOOP
     ...
     FOR v_recs IN    (SELECT table_name
                       FROM user_tables
                       WHERE table_name = l_tab_table_name(i)) LOOP
                       
               l_table_exists := TRUE;
     END LOOP;        

     IF NOT l_table_exists THEN
        RAISE_APPLICATION_ERROR(-20000,'Migration terminated because table '||l_tab_table_name(i)||' has not been migrated from source schema.');
     END IF;		
